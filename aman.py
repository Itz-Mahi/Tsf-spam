    ...
    if e.sender_id in SMEX_USERS:
        ...
        if len(e.text) > 6:
            ...
            try:
                ...
                await event.edit("𝗛𝗢 𝗚𝗬𝗔 𝗝𝗢𝗜𝗡 𝗔𝗕 𝗧𝗦𝗙 𝗠𝗔𝗔 𝗖𝗛𝗢𝗗𝗘𝗚𝗔 ")
            except Exception as e:
                await event.edit(str(e))   
        else:
            await e.reply(usage, parse_mode=None, link_preview=None )
...
async def ping(e):
    if e.sender_id in SMEX_USERS:
        ...
        await event.edit(f" 𝗧𝗦𝗙 !\n`{ms}` ms{ALIVE_NAME} "𝙅𝙊 𝙇𝙀𝙂𝘼 𝙏𝙎𝙁 𝙎𝙀 𝙆𝙃𝙐𝙎𝙆𝙄 𝙈𝘼𝘼 𝘾𝙃𝙊𝘿 𝘿𝙀𝙉𝙂𝙀 𝙐𝙎𝙆𝙄 👅👿)
...
text = """
𝙅𝙊 𝙇𝙀𝙂𝘼 𝙏𝙎𝙁 𝙎𝙀 𝙆𝙃𝙐𝙎𝙆𝙄 𝙈𝘼𝘼 𝘾𝙃𝙊𝘿 𝘿𝙀𝙉𝙂𝙀 𝙐𝙎𝙆𝙄 👅�
𝗕𝗬 𝗧𝗦𝗙 𝗥𝗔𝗣𝗦𝗧𝗔𝗥💥"""
